...
login.asset_base_url:
  description: Base url for static assets, allows custom styling of the login server.  Use
    '/resources/pivotal' for Pivotal style.
  default: /resources/oss
login.brand:
  description: The brand to use for the reset password emails, available values are
    oss and pivotal
  default: oss
login.enabled:
...
login.ldap.localPasswordCompare:
  description: Deprecated. Use uaa.ldap.localPasswordCompare - login.ldap prefix is
    used for backwards compatibility to enable ldap from login config
  default: "true"
login.ldap.passwordAttributeName:
  description: Deprecated. Use uaa.ldap.passwordAttributeName - login.ldap prefix
    is used for backwards compatibility to enable ldap from login config
  default: userPassword
login.ldap.passwordEncoder:
  description: Deprecated. Use uaa.ldap.passwordEncoder - login.ldap prefix is used
    for backwards compatibility to enable ldap from login config
  default: org.cloudfoundry.identity.uaa.login.ldap.DynamicPasswordComparator
login.ldap.profile_type:
  description: Deprecated. Use uaa.ldap.profile_type - login.ldap prefix is used for
    backwards compatibility to enable ldap from login config
  default: null
login.ldap.searchBase:
  description: Deprecated. Use uaa.ldap.searchBase - login.ldap prefix is used for
    backwards compatibility to enable ldap from login config
  default: ""
login.ldap.searchFilter:
  description: Deprecated. Use uaa.ldap.searchFilter - login.ldap prefix is used for
    backwards compatibility to enable ldap from login config
  default: cn={0}
login.ldap.sslCertificate:
  description: Deprecated. Use uaa.ldap.sslCertificate - login.ldap prefix is used
    for backwards compatibility to enable ldap from login config
  default: null
login.ldap.sslCertificateAlias:
  description: Deprecated. Use uaa.ldap.sslCertificateAlias - login.ldap prefix is
    used for backwards compatibility to enable ldap from login config
  default: null
login.ldap.url:
  description: Deprecated. Use uaa.ldap.url - login.ldap prefix is used for backwards
    compatibility to enable ldap from login config
  default: null
login.ldap.userDN:
  description: Deprecated. Use uaa.ldap.userDN - login.ldap prefix is used for backwards
    compatibility to enable ldap from login config
  default: null
login.ldap.userDNPattern:
  description: Deprecated. Use uaa.ldap.userDNPattern - login.ldap prefix is used
    for backwards compatibility to enable ldap from login config
  default: null
login.ldap.userPassword:
  description: Deprecated. Use uaa.ldap.userPassword - login.ldap prefix is used for
    backwards compatibility to enable ldap from login config
  default: null
login.links:
...
login.saml.entity_base_url:
  description: 'The URL for which SAML identity providers will post assertions to.
    If set it overrides the default. This URL should NOT have the schema (http://
    or https:// prefix in it) instead just the hostname. The schema is derived by
    login.protocol property. The default value is #{protocol}://#{properties.uaa.url}
    if uaa.url is set.'
  default: null
login.saml.entityid:
  description: 'This is used as the SAML Service Provider Entity ID. Each '
  default: null
...
login.signups_enabled:
  description: Deprecated. Use login.self_service_links_enabled. Instructs UAA to
    use 'enable account creation flow'. Enabled by default.
  default: true
login.smtp:
...
login.spring_profiles:
  description: Deprecated. Use uaa.ldap.enabled - login.spring_profiles is used for
    backwards compatibility to enable ldap from login config
  default: null
login.tiles:
...
route_registrar.routes:
  description: |
    ...
    * 'healthcheck.timeout' is optional, and is measured in seconds.
      The healthcheck timeout defaults to 0, which corresponds to a timeout
      of infinite duration. Values less than zero are not permitted.
  default: null
...
router.router_configurer.tcp_stats_unix_socket:
  description: Unix socket exposed by the the underlying tcp proxy to gather statistic
    metrics from
...
uaa.restricted_ips_regex:
  description: A pipe delimited set of regular expressions of IP addresses that can
    reach the listening HTTP port of the server.
  default: 10\.\d{1,3}\.\d{1,3}\.\d{1,3}|192\.168\.\d{1,3}\.\d{1,3}|169\.254\.\d{1,3}\.\d{1,3}|127\.\d{1,3}\.\d{1,3}\.\d{1,3}|172\.1[6-9]{1}\.\d{1,3}\.\d{1,3}|172\.2[0-9]{1}\.\d{1,3}\.\d{1,3}|172\.3[0-1]{1}\.\d{1,3}\.\d{1,3}
uaa.scim.external_groups:
  ...
  default: null
uaa.scim.groups:
  description: "Contains a hash of group names and their descriptions. These groups
    will be added to the UAA database for the default zone but not associated with
    any user.\nExample:\n  uaa:\n    scim:\n      groups:\n        my-test-group:
    'My test group description'\n        another-group: 'Another group description'\nDeprecated
    format(still supported, but may be removed in the future): \nComma separated list
    of groups that should be added to the UAA db, but not assigned to a user by default.\n"
  default: null
...
uaa.scim.users:
  ...
  default: null
uaa.spring_profiles:
  description: Deprecated. Use 'uaa.ldap.enabled' instead. Sets the Spring profiles
    on the UAA web application. This gets combined with the 'uaadb.db_scheme' property
